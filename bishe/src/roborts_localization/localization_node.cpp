...
bool LocalizationNode::Init() {
  ...
  // printf("\n\n__localizationstart__\n\n");
  odom_frame_ = std::move(localization_config.odom_frame_id);  //填上自己的ns
  global_frame_ = std::move(localization_config.global_frame_id);
  base_frame_ = std::move(localization_config.base_frame_id);
  // printf("\n\nns:%s\n\n",nh_.getNamespace().c_str());
  laser_topic_ = std::move(localization_config.laser_topic_name);  //无前缀
  std::string laser_topic_temp = laser_topic_;  //放原来的名字，先不加前缀
  if (nh_.getNamespace().c_str() != "/" &&
      nh_.getNamespace().size() >
          5)  //如果是多机器人，默认的命名空间就是/robot_x不是/
  {
    ...
    // laser_topic_ = nh_.getNamespace().substr(1) + '/' +
    // laser_topic_;//加前缀/
    // printf("\n\nbase_frame_:%s\n\n",base_frame_.c_str());
    // printf("\n\nglobal_frame_:%s\n\n",global_frame_.c_str());
    // printf("\n\nodom_frame_:%s\n\n",odom_frame_.c_str());
  ...
  distance_map_pub_ =
      nh_.advertise<nav_msgs::OccupancyGrid>("distance_map", 1, true);
  particlecloud_pub_ =
      nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);

  // Use message filter for time synchronizer (laser scan topic and tf between
  // odom and base frame)
  laser_scan_sub_ =
      std::make_shared<message_filters::Subscriber<sensor_msgs::LaserScan>>(
          nh_, laser_topic_temp, 100);  //这块自动加，所以使用没前缀的
  laser_scan_filter_ =
      std::make_unique<tf::MessageFilter<sensor_msgs::LaserScan>>(
          *laser_scan_sub_, *tf_listener_ptr_, odom_frame_, 100);
  laser_scan_filter_->registerCallback(
      boost::bind(&LocalizationNode::LaserScanCallback, this, _1));

  initial_pose_sub_ = nh_.subscribe(
      "initialpose", 2, &LocalizationNode::InitialPoseCallback, this);
  pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("amcl_pose", 2, true);
  // static_map_sub=nh_.subscribe<nav_msgs::OccupancyGrid>("/map",1,&LocalizationNode::StaticMapCallback,this);
  amcl_ptr_ = std::make_unique<Amcl>();
  amcl_ptr_->GetParamFromRos(&nh_);
  amcl_ptr_->Init(init_pose_, init_cov_);
  // ros::Rate temp_rate(10);
  //   while(!first_map_received_) {//等staticmap接到消息
  //       ros::spinOnce();
  //       temp_rate.sleep();
  //       // printf("\n\n__gettingmap__\n\n");
  //   }
...
void LocalizationNode::StaticMapCallback(
    const nav_msgs::OccupancyGrid::ConstPtr &staticmap) {
  staticmap_temp_ = *staticmap;
  first_map_received_ = true;
}
bool LocalizationNode::GetStaticMap() {
  // while(ros::ok()){
  //   if(first_map_received_ == true){
  // amcl_ptr_->HandleMapMessage(staticmap_temp_, init_pose_, init_cov_);
  // return true;
  // }
...
bool LocalizationNode::GetLaserPose() {  //应该是用来得到初始tf的
  laser_topic_ = '/' + nh_.getNamespace().substr(1) + '/' + laser_topic_;
  // printf("\n\nlasertopicname:%s\n\n",laser_topic_.c_str());//应该是什么？

  auto laser_scan_msg =
      ros::topic::waitForMessage<sensor_msgs::LaserScan>(laser_topic_);
  // printf("\n\nlasermsgheaderframeid:%s\n\n",laser_scan_msg->header.frame_id.c_str());
  Vec3d laser_pose;
  laser_pose.setZero();
  GetPoseFromTf(base_frame_, laser_scan_msg->header.frame_id, ros::Time(),
                laser_pose);
  laser_pose[2] = 0;  // No need for rotation, or will be error
  DLOG_INFO << "Received laser's pose wrt robot: " << laser_pose[0] << ", "
            << laser_pose[1] << ", " << laser_pose[2];
...
void LocalizationNode::InitialPoseCallback(
    const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &init_pose_msg) {
  printf("\n\n__getinitalpose__\n\n");  //一直没接到
  ...
  try {
    ...
    tf_listener_ptr_->waitForTransform(base_frame_, init_pose_msg->header.stamp,
                                       base_frame_, now, odom_frame_,
                                       ros::Duration(0.5));
    tf_listener_ptr_->lookupTransform(base_frame_, init_pose_msg->header.stamp,
                                      base_frame_, now, odom_frame_, tx_odom);
...
void LocalizationNode::LaserScanCallback(
    const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg_ptr) {
  ...
  amcl_ptr_->Update(pose_in_odom, laser_scan_msg, angle_min, angle_increment,
                    particlecloud_msg_, hyp_pose_);
...
void LocalizationNode::PublishVisualize() {
  if (pose_pub_.getNumSubscribers() > 0) {
    ...
    pose_msg_.pose.orientation =
        tf::createQuaternionMsgFromYaw(hyp_pose_.pose_mean[2]);
...
bool LocalizationNode::PublishTf() {
  ...
  if (amcl_ptr_->CheckTfUpdate()) {
    ...
    try {
      tf::Transform tmp_tf(
          tf::createQuaternionFromYaw(hyp_pose_.pose_mean[2]),
          tf::Vector3(hyp_pose_.pose_mean[0], hyp_pose_.pose_mean[1], 0.0));
      tf::Stamped<tf::Pose> tmp_tf_stamped(
          tmp_tf.inverse(), last_laser_msg_timestamp_, base_frame_);
      this->tf_listener_ptr_->transformPose(odom_frame_, tmp_tf_stamped,
                                            odom_to_map);
    } catch (tf::TransformException &e) {
    ...
    tf::StampedTransform tmp_tf_stamped(
        latest_tf_.inverse(), transform_expiration, global_frame_, odom_frame_);
...
bool LocalizationNode::GetPoseFromTf(const std::string &target_frame,
                                     ...
                                     const ros::Time &timestamp, Vec3d &pose) {
  ...
  try {
    ...
    LOG_ERROR << "Couldn't transform from " << source_frame << "to "
              << target_frame;
    return false;
  }
...
void LocalizationNode::TransformLaserscanToBaseFrame(
  ...
  try {
    ...
    LOG_WARNING << "Unable to transform min/max laser angles into base frame: "
                << e.what();
    return;
  }
