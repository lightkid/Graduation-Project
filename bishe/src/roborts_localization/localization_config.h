...
struct LocalizationConfig {
  void GetParam(ros::NodeHandle *nh) {
      nh->param<std::string>("odom_frame", odom_frame_id, "odom");//一定要动，每个机器人有自己的robot_x/odom
      nh->param<std::string>("base_frame", base_frame_id, "base_link");//也动，跟odom一样
      nh->param<std::string>("global_frame", global_frame_id, "map");//应该不用动，几个机器人共享
    ...
    if(nh->getNamespace().c_str()!="/"&&nh->getNamespace().size()>5)
      {
        initial_pose_x_str = initial_pose_x_str + '_' + nh->getNamespace().substr(1);
        initial_pose_y_str = initial_pose_y_str + '_' + nh->getNamespace().substr(1);
        initial_pose_a_str = initial_pose_a_str + '_' + nh->getNamespace().substr(1);
      ...
      ROS_INFO("%sinital_pose=(%f,%f,%f)",initial_pose_x_str.c_str(),initial_pose_x,initial_pose_y,initial_pose_a);
