...
typedef double (*lbfgs_evaluate_t)(
    void *instance,
    const Eigen::VectorXd &x,  //该x有x0和xn,x0x1x2...xny0y1y2...yn(z...)
    Eigen::VectorXd &g);       // g的维度比x少4维
...
inline int line_search_lewisoverton(
  ...
  // int ret = 1;
  const int n = x.size();
  double l = param.min_step;
  double u = param.max_step;
  double alpha = 1;
  int cnt = param.max_linesearch;
  double fstp;
  Eigen::VectorXd gstp(n);
  while (cnt--) {
    fstp = cd.proc_evaluate(cd.instance, x + alpha * s, gstp);
    if (f - fstp < -param.f_dec_coeff * alpha * (s.dot(gp))) {
      u = alpha;
    } else if (s.dot(gstp) < param.s_curv_coeff * (s.dot(gp))) {
      l = alpha;
    } else {
      // stp = alpha;
      break;
