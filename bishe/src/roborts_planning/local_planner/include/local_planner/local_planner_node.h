...
class LocalPlannerNode {
 public:
  ...
  void GlobalCallback(const nav_msgs::Path::ConstPtr &path);
  void PathCallback(const nav_msgs::Path::ConstPtr &path);
  void AimCallback(const geometry_msgs::PoseStamped::ConstPtr & aim);
  void OdomCallback(const geometry_msgs::PoseStamped::ConstPtr & odom);
  /**
 ...
 private:
  ...
  ros::Subscriber global_sub_;//get path from global_planner_node//new接到path就把pathCB打断
  // ros::Publisher global_pub_;
  ros::Subscriber path_sub_;//get path from global_planner_node//new
  ros::Subscriber aim_sub_;//接要瞄准的敌方车位置，然后做角度规划用
  ros::Subscriber odom_sub_;
  ros::Publisher traget_pub_;
  geometry_msgs::PoseStamped backpos_;
  bool has_aim;
  bool decouple;//解耦控制
  // bool aim;//对准控制
  geometry_msgs::PoseStamped aimpos_;//瞄准的地方位置，传入teb用
  bool new_path_local_;//打断用标志//名字与global中的重复？？
